...
private:
	...
	MM_HeapLinkedFreeHeader *_firstUnAlignmentFreeEntry; /**< it is only for Balanced GC copyforward and non empty survivor region */
	MM_HeapLinkedFreeHeader *_preFirstUnAlignmentFreeEntry;
protected:
public:
...
private:
	...
	{
		bool ret = false;
		if ((NULL != _firstUnAlignmentFreeEntry) && (NULL != freeEntry) && ((uintptr_t) freeEntry >= (uintptr_t)_firstUnAlignmentFreeEntry)) {
			ret = true;
		}
		return ret;
	}
	uintptr_t getAlignedSize(MM_EnvironmentBase *env, void *addrBase, void *addrTop);
	MMINLINE bool isAlignedSizeFit(MM_EnvironmentBase *env, MM_HeapLinkedFreeHeader *freeEntry, uintptr_t requiredSize)
	{
		bool ret = false;
		if (getAlignedSize(env, (void *)freeEntry, (void *)((uintptr_t)freeEntry + freeEntry->getSize())) >= requiredSize) {
			ret = true;
		}
		return ret;
	}

	MM_HeapLinkedFreeHeader *doFreeEntryAlignmentUpTo(MM_EnvironmentBase *env, MM_HeapLinkedFreeHeader *freeEntry);
...
public:
	...
	MMINLINE void initialFirstUnAlignmentFreeEntry()
	{
		_firstUnAlignmentFreeEntry = _heapFreeList;
		_preFirstUnAlignmentFreeEntry = NULL;
	}

	MMINLINE void resetFirstUnAlignmentFreeEntry()
	{
...
//	void reportMemoryPool(MM_EnvironmentBase *env);
//	bool verifyFreeSizeCount(MM_EnvironmentBase *env);
