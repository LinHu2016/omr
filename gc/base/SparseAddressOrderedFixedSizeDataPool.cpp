...
bool
MM_SparseAddressOrderedFixedSizeDataPool::verifyAndUnmapSparseDataPtrFromHeapProxyObjectPtr(void *dataPtr, void *proxyObjPtr, uintptr_t size)
{
	bool ret = true;
	MM_SparseDataTableEntry entryToRemove = MM_SparseDataTableEntry(dataPtr);
...
bool
MM_SparseAddressOrderedFixedSizeDataPool::validateSparseDataEntry(MM_SparseDataTableEntry *entry, void *dataPtr, void *proxyObjPtr, uintptr_t size)
{
	bool ret = false;
...
bool
MM_SparseAddressOrderedFixedSizeDataPool::verifyAndUpdateSparseDataEntryAfterObjectHasMoved(void *dataPtr, void *proxyObjPtr, uintptr_t size, void *newProxyObjPtr)
{
	MM_SparseDataTableEntry lookupEntry = MM_SparseDataTableEntry(dataPtr);
	MM_SparseDataTableEntry *entry = (MM_SparseDataTableEntry *)hashTableFind(_objectToSparseDataTable, &lookupEntry);

	Assert_MM_true((NULL != entry) && verifySparseDataEntry(entry, dataPtr, proxyObjPtr, size));
	Trc_MM_SparseAddressOrderedFixedSizeDataPool_updateEntry_success(dataPtr, entry->_proxyObjPtr, newProxyObjPtr);
